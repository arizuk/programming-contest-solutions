...
pub mod ds {
    ...
    where
        T: Copy
    {
        pub fn new<F>(size: usize, init: T, f: &'a F) -> Self
        where
            F: Fn(T, T) -> T
        {
            let mut n = 1;
...
use ds::SegmentTree;
// type SegmentTree = ds::SegmentTree<usize, F: Fn(usize,usize) -> usize>;
...
fn main() {
    ...
    for i in 0..n {
        let seg = if i%2 == 0 { &mut seg0 } else { &mut seg1 };
        seg.set(i, (ps[i], i));
    }
