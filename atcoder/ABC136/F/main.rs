...
fn main() {
    ...
    for i in 0..n {
        let mut o = vec![0; 4];
        for j in 0..4 {
            o[j] = mod_pow(2, points[i][j]);
        }
        // a&&d || b&&c - a&&b&&c&&d
        let mut temp = 0;
        temp += (o[0]-1) %MOD * o[1]%MOD * o[2]%MOD * (o[3]-1)%MOD;
        temp %= MOD;
        temp += o[0] * (o[1]-1)%MOD * (o[2]-1)%MOD * o[3] %MOD;
        temp %= MOD;
        temp += MOD;
        temp -= (o[0]-1) * (o[1]-1)%MOD * (o[2]-1)%MOD * (o[3]-1)%MOD;
        temp %= MOD;
        temp += o[0] * o[1]%MOD * o[2]%MOD * o[3]%MOD;
        temp %= MOD;
        ans += temp;
        ans %= MOD;
    }
